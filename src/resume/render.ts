...
/**
 * Neutralises text that a poster controls before it is written into Markdown.
 *
 * A job title containing `![x](http://attacker/ping.png)` becomes a network
 * request the moment the user previews the file, which leaks their IP and the
 * fact that they are reading it — from a document generated by a tool whose
 * rule is that a posting must never cause a network request. Claim text is
 * already rejected for markup by the validator; this covers the fields that
 * come straight from the posting.
 */
